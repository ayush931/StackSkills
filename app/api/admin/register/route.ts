...
export async function POST(request: NextRequest) {
  try {
    ...
    const { name, email, password, phone, adminToken } = requestBody;

    const ADMIN_SECRET = process.env.ADMIN_REGISTRATION_TOKEN

    if (adminToken !== ADMIN_SECRET) {
      return NextResponse.json(
        { success: false, message: 'Provide ADMIN token' }
      )
    }
    ...
    if (!rateLimitCheck.allowed) {
      return NextResponse.json(
        { success: false, message: 'Too many admin registration attempts. Please try again', resetTime: rateLimitCheck.resetTime },
        { status: 429 }
      )
    }
