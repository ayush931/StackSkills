...
export const hashPassword = async (userPassword: string): Promise<{
  success: boolean,
  hash?: string;
  error?: string
}> => {
...
export const comparePassword = async (userPassword: string, dbPassword: string): Promise<{
  success: boolean;
  isMatch?: boolean;
  error?: string
}> => {
  try {
    if (!userPassword || !dbPassword || typeof userPassword !== "string" || typeof dbPassword !== "string") {
      await new Promise(resolve => setTimeout(resolve, crypto.randomInt(100, 500)))
